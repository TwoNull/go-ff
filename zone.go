...
func ParseZoneData(decompressedData []byte) *ZoneData {
	...
	zd.g_streamOutSize = getDword(decompressedData, 0, true)

	for i := 0; i < 9; i++ {
		zd.g_streamBlockSize[i] = getDword(decompressedData, 4*(i+1), true)
	}

	zd.listStringCount = getDword(decompressedData, 0x2C, true)

	zd.assetsCount = getDword(decompressedData, 0x24, true)
	...
	for i := 0; i < int(zd.listStringCount); i++ {
		strOffset := getDword(decompressedData, 0x3C+4*i, true)
		if strOffset == 0xFFFFFFFF {
			listStringStrEndOffset = findByte(decompressedData, 0x00, listStringStrStartOffset)
			zd.listStrings[i] = getString(decompressedData, listStringStrStartOffset, listStringStrEndOffset)
	...
	for i := 0; i < zd.assetsCount; i++ {
		assetType := getDword(decompressedData, zd.assetsListOffset+8*i, true)
